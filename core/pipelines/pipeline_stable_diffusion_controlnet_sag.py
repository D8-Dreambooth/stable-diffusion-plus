...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import StableDiffusionControlNetSAGPipeline

        >>> pipe = StableDiffusionControlNetSAGPipeline.from_pretrained(
...
def check_image(image, prompt, prompt_embeds):
    ...
    if not image_is_pil and not image_is_tensor and not image_is_pil_list and not image_is_tensor_list:
        raise TypeError(
            "image must be passed and be one of PIL image, torch tensor, list of PIL images, or list of torch "
            "tensors"
        )
    image_batch_size = 1
    prompt_batch_size = 1
...
class StableDiffusionControlNetSAGPipeline(DiffusionPipeline, TextualInversionLoaderMixin, PipelineOptimMixin):
    ...
    def __call__(
        ...
        height, width = _default_height_width(height, width, image)
