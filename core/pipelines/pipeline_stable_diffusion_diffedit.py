...
class StableDiffusionDiffEditPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def check_inputs(
        ...
        if mask_prompt is None and mask_embeds is None:
            raise ValueError(
                "Provide either `mask_prompt` or `mask_embeds`. Cannot leave both `mask_prompt` and `mask_embeds`"
                " undefined."
            )
        elif mask_prompt is not None and mask_embeds is not None:
            raise ValueError(
                f"Cannot forward both `mask_prompt`: {mask_prompt} and `mask_embeds`: {mask_embeds}. Please make sure"
    ...
    def check_source_inputs(
        ...
        if source_prompt is not None and source_prompt_embeds is not None:
            ...
            raise ValueError(
                "Provide either `source_image` or `source_prompt_embeds`. Cannot leave all both of the arguments "
                "undefined."
            )
    ...
    def __call__(
        ...
        Args:
            ...
            mask_prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts to guide the inpainting process. If not defined, one has to pass `mask_embeds`.
            image (`PIL.Image.Image`):
            ...
            prompt_embeds (`torch.FloatTensor`, *optional*):
                Pre-generated text embeddings. Can be used to easily tweak text inputs, *e.g.* prompt weighting. If not
                provided, text embeddings will be generated from `prompt` input argument.
            mask_embeds (`torch.FloatTensor`, *optional*):
                Pre-generated mask text embeddings. Can be used to easily tweak text inputs, *e.g.* prompt weighting.
            negative_prompt_embeds (`torch.FloatTensor`, *optional*):
                Pre-generated negative text embeddings. Can be used to easily tweak text inputs, *e.g.* prompt
                weighting. If not provided, negative_prompt_embeds will be generated from `negative_prompt` input
                argument.
        ...
        mask_image = self.generate_mask(image=image, source_prompt=prompt, target_prompt=mask_prompt,
                                        target_prompt_embeds=mask_embeds)
        image_latents = self.invert(
            image=image,
            prompt=mask_prompt,
            prompt_embeds=mask_embeds,
