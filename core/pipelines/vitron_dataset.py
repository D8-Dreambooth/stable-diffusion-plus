...
class CPDataset(data.Dataset):
    ...
    def __init__(
        self,
        person_images,    # List of PIL Images for persons
        person_captions,  # Corresponding list of captions for persons
        garment_image,    # PIL Image for garment
        garment_caption,  # Caption for garment
        opt
    ):
        ...
        # load data list
        self.im_names = person_captions
        self.person_images = person_images
        self.garment_image = garment_image
        self.garment_caption = garment_caption

        self.c_names = dict()
        self.c_names["paired"] = person_captions
        self.c_names["unpaired"] = [garment_caption]
    ...
    def get_agnostic(self, im, im_parse, pose_data):
        ...
        agnostic_draw.polygon(
            [tuple(pose_data[i]) for i in [2, 5, 12, 9]], "gray", "gray"
        )
        ...
        for i in [3, 4, 6, 7]:
            if (pose_data[i - 1, 0] == 0.0 and pose_data[i - 1, 1] == 0.0) or (
                ...
                continue
            agnostic_draw.line(
                [tuple(pose_data[j]) for j in [i - 1, i]], "gray", width=r * 10
            )
            pointx, pointy = pose_data[i]
            agnostic_draw.ellipse(
    ...
    def __getitem__(self, index):
        ...
        cm = {}
        for key in ["paired"]:
            c_name[key] = self.garment_caption
            c[key] = self.garment_image
            c[key] = transforms.Resize(self.fine_width, interpolation=2)(c[key])
            cm[key] = self.transform(c[key])  # [-1,1]

            c[key] = self.transform(c[key])  # [-1,1]
            cm_array = np.array(c[key])
            cm_array = (cm_array >= 128).astype(np.float32)
            cm[key] = torch.from_numpy(cm_array)  # [0,1]
            cm[key].unsqueeze_(0)

        # person image
        im_pil_big = self.person_images[index]
        ...
        im_parse_pil = transforms.Resize(self.fine_width, interpolation=0)(
            im_parse_pil_big
        )
        ...
        new_parse_map = torch.FloatTensor(
            self.semantic_nc, self.fine_height, self.fine_width
        ).zero_()
        ...
        image_parse_agnostic = Image.open(
            osp.join(
                self.data_path,
                parse_name.replace("image-parse-v3", "image-parse-agnostic-v3.2"),
            )
        )
        image_parse_agnostic = transforms.Resize(self.fine_width, interpolation=0)(
        ...
        parse_agnostic_map = torch.FloatTensor(
            20, self.fine_height, self.fine_width
        ).zero_()
        ...
        # load pose points
        pose_name = im_name.replace("image", "openpose_img").replace(
            ".jpg", "_rendered.png"
        )
        ...
        # pose name
        pose_name = im_name.replace("image", "openpose_json").replace(
            ".jpg", "_keypoints.json"
        )
        ...
        densepose_map = transforms.Resize(self.fine_width, interpolation=2)(
            densepose_map
        )
